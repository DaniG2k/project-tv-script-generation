...
def test_create_lookup_tables(create_lookup_tables):
    ...
    assert not (vocab_to_int_word_set - int_to_vocab_word_set), 'vocab_to_int and int_to_vocab don\'t have the same words.' \
    ...
    assert not missmatches, 'Found {} missmatche(s). First missmatch: vocab_to_int[{}] = {} and int_to_vocab[{}] = {}'.format(len(missmatches), *missmatches[0])
    
    assert len(vocab_to_int) > len(set(test_text))/2, 'The length of vocab seems too small.  Found a length of {}'.format(len(vocab_to_int))
...
def test_tokenize(token_lookup):
    ...
    assert not symbol_val, 'Don\'t use a symbol that will be replaced in your tokens. Found the symbol {} in value {}'.format(*symbol_val)
